...
static int32_t test_calc_crc_init_null_handle(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_ERROR_INPUT_ARGS, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", NULL,
...
static int32_t test_calc_crc_init_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", NULL, &conf,
                  CALC_CRC_STATUS_OK, result);
    CTEST_ASSERT_EQ_MSG(false, crc.update_in_progress, "Expected %d, but got %d.", false, crc.update_in_progress);
    CTEST_ASSERT_EQ_MSG(0, crc.update_chunk, "Expected %d, but got %d.", 0, crc.update_chunk);
    CTEST_ASSERT_EQ_MSG(0, crc.update_crc, "Expected %d, but got %d.", 0, crc.update_crc);
    CTEST_ASSERT_EQ_MSG(0, crc.update_crc_final, "Expected %d, but got %d.", 0, crc.update_crc_final);
    CTEST_ASSERT_EQ_MSG(&conf, crc.conf, "Expected %d, but got %d.", &conf, crc.conf);
...
static int32_t test_calc_crc_calculate_null_handle(void)
{
    ...
    result = CALC_CRC_Init(&crc, &conf, CALC_CRC_TYPE_8BIT);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", &crc, &conf,
                  CALC_CRC_STATUS_OK, result);

    // Test NULL CRC handle
    result = CALC_CRC_Calculate(NULL, data, sizeof(data), &crc_result);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_ERROR_INPUT_ARGS, result,
...
static int32_t test_calc_crc_calculate_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", &crc, &conf_8bit,
                  CALC_CRC_STATUS_OK, result);

    result = CALC_CRC_Calculate(&crc, data, data_length, &result_8bit);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_8bit);
    CTEST_ASSERT_EQ_MSG(0x4, result_8bit, "Expected %08X, but got %02X.", 0x4, result_8bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", &crc, &conf_16bit,
                  CALC_CRC_STATUS_OK, result);

    result = CALC_CRC_Calculate(&crc, data, data_length, &result_16bit);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_16bit);
    CTEST_ASSERT_EQ_MSG(0xD64E, result_16bit, "Expected %08X, but got %04X.", 0xD64E, result_16bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", &crc, &conf_32bit,
                  CALC_CRC_STATUS_OK, result);

    result = CALC_CRC_Calculate(&crc, data, data_length, &result_32bit);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_32bit);
    CTEST_ASSERT_EQ_MSG(0xFC891918, result_32bit, "Expected %08X, but got %08X.", 0xFC891918, result_32bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Init(%p, %p) -> Expected %d, but got %d.", &crc, &conf_64bit,
                  CALC_CRC_STATUS_OK, result);

    result = CALC_CRC_Calculate(&crc, data, data_length, &result_64bit);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_64bit);
    CTEST_ASSERT_EQ_MSG(0x62EC59E3F1A4F00A, result_64bit, "Expected %08X, but got %04X.", 0x62EC59E3F1A4F00A, result_64bit);
...
static int32_t test_calc_crc_calculate_8bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_8bit);
    CTEST_ASSERT_EQ_MSG(0xF4, result_8bit, "Expected %02X, but got %02X.", 0xF4, result_8bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_8bit);
    CTEST_ASSERT_EQ_MSG(0x20, result_8bit, "Expected %02X, but got %02X.", 0x20, result_8bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_8bit);
    CTEST_ASSERT_EQ_MSG(0x2F, result_8bit, "Expected %02X, but got %02X.", 0x2F, result_8bit);
...
static int32_t test_calc_crc_calculate_16bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_16bit);
    CTEST_ASSERT_EQ_MSG(0x31C3, result_16bit, "Expected %08X, but got %04X.", 0x31C3, result_16bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_16bit);
    CTEST_ASSERT_EQ_MSG(0x2189, result_16bit, "Expected %08X, but got %04X.", 0x2189, result_16bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_16bit);
    CTEST_ASSERT_EQ_MSG(0xC38C, result_16bit, "Expected %08X, but got %04X.", 0xC38C, result_16bit);
...
static int32_t test_calc_crc_calculate_32bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_32bit);
    CTEST_ASSERT_EQ_MSG(0x89A1897F, result_32bit, "Expected %08X, but got %08X.", 0x89A1897F, result_32bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_32bit);
    CTEST_ASSERT_EQ_MSG(0x2DFD2D88, result_32bit, "Expected %08X, but got %08X.", 0x2DFD2D88, result_32bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_32bit);
    CTEST_ASSERT_EQ_MSG(0xFE918591, result_32bit, "Expected %08X, but got %08X.", 0xFE918591, result_32bit);
...
static int32_t test_calc_crc_calculate_64bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_64bit);
    CTEST_ASSERT_EQ_MSG(0x6C40DF5F0B497347, result_64bit, "Expected %08X, but got %04X.", 0x6C40DF5F0B497347, result_64bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_64bit);
    CTEST_ASSERT_EQ_MSG(0x2B9C7EE4E2780C8A, result_64bit, "Expected %08X, but got %04X.", 0x2B9C7EE4E2780C8A, result_64bit);
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result_64bit);
    CTEST_ASSERT_EQ_MSG(0xE2CE92D0FAFB0236, result_64bit, "Expected %08X, but got %04X.", 0xE2CE92D0FAFB0236, result_64bit);
...
static int32_t test_calc_crc_update_8bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(0x4, output, "Expected %02X, but got %02X.", 0x4, output);
    CTEST_ASSERT_EQ_MSG(0, crc.update_chunk, "Expected %d, but got %d.", 0, crc.update_chunk);
    CTEST_ASSERT_EQ_MSG(false, crc.update_in_progress, "Expected %d, but got %d.", false, crc.update_in_progress);
    CTEST_ASSERT_EQ_MSG(&conf, crc.conf, "Expected %d, but got %d.", &conf, crc.conf);

    result = CALC_CRC_Update(&crc, data, data_length - 4);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Update(%p, %p, %d) -> Expected %d, but got %d.", &crc, data,
                  data_length);
    CTEST_ASSERT_EQ_MSG(true, crc.update_in_progress, "Expected %d, but got %d.", true, crc.update_in_progress);
    CTEST_ASSERT_EQ_MSG(1, crc.update_chunk, "Expected %d, but got %d.", 1, crc.update_chunk);
    CTEST_ASSERT_EQ_MSG(&conf, crc.conf, "Expected %d, but got %d.", &conf, crc.conf);

    result = CALC_CRC_Update(&crc, data + (data_length - 4), 4);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Update(%p, %p, %d) -> Expected %d, but got %d.", &crc, data,
                  data_length);
    CTEST_ASSERT_EQ_MSG(2, crc.update_chunk, "Expected %d, but got %d.", 2, crc.update_chunk);
    CTEST_ASSERT_EQ_MSG(&conf, crc.conf, "Expected %d, but got %d.", &conf, crc.conf);

    result = CALC_CRC_Finalize(&crc, &output);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Finalize(%p, %p) -> Expected %d, but got %d.", &crc, result);
    CTEST_ASSERT_EQ_MSG(false, crc.update_in_progress, "Expected %d, but got %d.", false, crc.update_in_progress);
    CTEST_ASSERT_EQ_MSG(0x4, output, "Expected %02X, but got %02X.", 0x4, output);
    CTEST_ASSERT_EQ_MSG(2, crc.update_chunk, "Expected %d, but got %d.", 2, crc.update_chunk);
    CTEST_ASSERT_EQ_MSG(&conf, crc.conf, "Expected %d, but got %d.", &conf, crc.conf);
...
static int32_t test_calc_crc_update_16bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result);
    CTEST_ASSERT_EQ_MSG(0xD64E, output, "Expected %04X, but got %04X.", 0xD64E, output);
...
static int32_t test_calc_crc_update_32bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result);
    CTEST_ASSERT_EQ_MSG(0xFC891918, output, "Expected %08X, but got %08X.", 0xFC891918, output);
...
static int32_t test_calc_crc_update_64bit_valid(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Calculate(%p, %p, %d, %p) -> Expected %d, but got %d.", &crc,
                  data, data_length, result);
    CTEST_ASSERT_EQ_MSG(0x62EC59E3F1A4F00A, output, "Expected %013X, but got %016X.", 0x62EC59E3F1A4F00A, output);
    ...
    result = CALC_CRC_Finalize(&crc, &output);
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_OK, result, "CALC_CRC_Finalize(%p, %p) -> Expected %d, but got %d.", &crc, result);
    CTEST_ASSERT_EQ_MSG(0x62EC59E3F1A4F00A, output, "Expected %016X, but got %016X.", 0x62EC59E3F1A4F00A, output);
...
static int32_t test_calc_crc_update_null_handle(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_ERROR_INPUT_ARGS, result, "CALC_CRC_Update(%p, %p, %zu) -> Expected %d, but got %d.",
...
static int32_t test_calc_crc_finalize_null_handle(void)
{
    ...
    CTEST_ASSERT_EQ_MSG(CALC_CRC_STATUS_ERROR_INPUT_ARGS, result, "CALC_CRC_Finalize(%p, %p) -> Expected %d, but got %d.",
